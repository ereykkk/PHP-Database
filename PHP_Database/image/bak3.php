...
// Replace the existing command terminal handling code with this:
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'execute_command') {
    ...
    if (!empty($command)) {
        ...
        chdir($root_directory . $currentDir);

        // Handle 'ls' command for Windows
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            $command = str_replace('ls', 'dir', $command);
        }

        // Execute the command
        $fullCommand = "cd " . escapeshellarg(getcwd()) . " && " . $command . " 2>&1";
        ...
        // Add current working directory to the output
...
<body class="bg-light">
                    ...
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <div class="mb-3 d-flex justify-content-between align-items-center">
                                ...
                                <button id="openCommandModal" class="btn btn-dark" title="Open Command Terminal">
                                    ...
                                    <tbody>
                                        <?php foreach ($files as $file): ?>
                                            <?php if ($file != "." && $file != ".."): ?>
                                                ...
                                                $fileSize = $isDir ? '-' : formatFileSize($filePath);
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mt-3">
              <input type="text" class="form-control" id="modalCommandInput" placeholder="Enter command">
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" id="clearCommandOutput">Clear</button>
            <button type="button" class="btn btn-primary" id="modalExecuteCommand">Execute</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Command terminal
            $('#openCommandModal').click(function() {
                var commandOutputModal = new bootstrap.Modal(document.getElementById('commandOutputModal'));
                commandOutputModal.show();
            });

            // Command terminal (inside modal)
            $('#modalExecuteCommand').click(function() {
                const command = $('#modalCommandInput').val();
                if (command) {
            ...
            function executeCommand(command) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#modalCommandOutput').html($('#modalCommandOutput').html() + '<strong>> ' + escapeHtml(command) + '</strong><br>' + output + '<br><br>');
                        
                        // Clear the input
                        $('#modalCommandInput').val('');
                        
                        // Scroll to the bottom of the output
                        var outputElement = document.getElementById('modalCommandOutput');
            ...
            $('#modalCommandInput').keypress(function(e) {
                if (e.which == 13) {
                    $('#modalExecuteCommand').click();
                    return false;
                }
            });
            ...
            // Add this new event listener for the Clear button
            $('#clearCommandOutput').click(function() {
                $('#modalCommandOutput').empty();
            });
        });
    </script>
</body>
